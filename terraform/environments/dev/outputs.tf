...
output "services_ssh_commands" {
  value = {
    users_service  = module.bastion.bastion_public_ip != null ? [
      for ip in data.aws_instances.users_service_instances.private_ips :
      "ssh -i ~/.ssh/selena-aws-key.pem -o ProxyCommand='ssh -i ~/.ssh/selena-aws-key.pem -W %h:%p ec2-user@${module.bastion.bastion_public_ip}' ec2-user@${ip}"
    ] : []

    hotels_service = module.bastion.bastion_public_ip != null ? [
      for ip in data.aws_instances.hotels_service_instances.private_ips :
      "ssh -i ~/.ssh/selena-aws-key.pem -o ProxyCommand='ssh -i ~/.ssh/selena-aws-key.pem -W %h:%p ec2-user@${module.bastion.bastion_public_ip}' ec2-user@${ip}"
    ] : []
  }
